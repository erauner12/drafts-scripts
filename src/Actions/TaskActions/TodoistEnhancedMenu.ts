...
export async function runTodoistEnhancedMenu(): Promise<void> {
  ...
  /**
   * manageOverdueTasksSegmented
   * Example of a specialized function to fetch and display only overdue tasks,
   * then offer actions that are meaningful to overdue tasks.
   */
  async function manageOverdueTasksSegmented() {
    log("manageOverdueTasksSegmented() invoked.");
    let overdueTasks = await todoist.getTasks({ filter: "overdue" });
    ...
    prompt.title = "Overdue Tasks";
    prompt.message = "Select overdue tasks to act on:";
    let taskContents = overdueTasks.map((t) => t.content);
    prompt.addSelect("selectedOverdue", "Overdue", taskContents, [], true);
    prompt.addButton("Update to Today");
    prompt.addButton("Move to Future");
    prompt.addButton("Remove Due Date");
    prompt.addButton("Complete Task");
    prompt.addButton("Skip");

    if (!prompt.show()) {
      log("User closed the Overdue tasks prompt.");
      return;
    }

    let selected = prompt.fieldValues["selectedOverdue"] || [];
    if (!Array.isArray(selected) || selected.length === 0) {
      showAlert("No Tasks Selected", "No overdue tasks selected.");
      return;
    }

    let chosenAction = prompt.buttonPressed;
    switch (chosenAction) {
      case "Update to Today":
        for (let t of overdueTasks) {
          if (selected.includes(t.content)) {
            await handleUpdateToToday(t);
          }
        }
        break;
      case "Move to Future":
      ...
      case "Complete Task":
        for (let t of overdueTasks) {
          if (selected.includes(t.content)) {
            await todoist.closeTask(t.id);
          }
        }
        break;
      case "Skip":
        log("User skipped managing overdue tasks.");
        break;
      default:
        log("User pressed an unexpected button for overdue tasks.");
    }
  }

  /**
   * manageTodayNoTimeTasksSegmented
   * Example of a specialized function for tasks that are due today but have no assigned time.
   * Offers only time or scheduling options that make sense for these tasks.
   */
  async function manageTodayNoTimeTasksSegmented() {
    log("manageTodayNoTimeTasksSegmented() invoked.");

    let todayStr = getTodayDate();
    let tasksDueToday = await todoist.getTasks({ filter: "due: today" });
    // Filter out only tasks that have no specific time assigned:
    let noTimeTasks = tasksDueToday.filter((t) => !t.due?.datetime);

    if (noTimeTasks.length === 0) {
      showAlert(
        "No Today Tasks Without Time",
        "All tasks for today have a time or there are none."
      );
      log("No tasks missing time.");
      return;
    }

    let prompt = new Prompt();
    prompt.title = "Tasks Due Today (No Time)";
    prompt.message = "Select tasks to assign a time or otherwise modify.";
    let taskContents = noTimeTasks.map((t) => t.content);
    prompt.addSelect("selectedNoTime", "NoTime", taskContents, [], true);
    prompt.addButton("Set Time");
    prompt.addButton("Remove Due Date");
    prompt.addButton("Skip");

    if (!prompt.show()) {
      log("User closed the No-Time tasks prompt.");
      return;
    }

    let selected = prompt.fieldValues["selectedNoTime"] || [];
    if (!Array.isArray(selected) || selected.length === 0) {
      showAlert("No Tasks Selected", "No tasks selected in this category.");
      return;
    }
    ...
    switch (chosenAction) {
      ...
      case "Remove Due Date":
        ...
        break;
      case "Skip":
        log("User skipped no-time tasks action.");
        break;
      default:
        log(
          "User pressed an unexpected button for tasks with no time assigned."
        );
    }
  }
...
/**
 * runSegmentedTasksMenu
 * A dedicated step which presents a menu to the user, allowing them to pick
 * which specialized flow they want to invoke (e.g., manage overdue tasks,
 * manage tasks due today with no time, etc.).
 */
export async function runSegmentedTasksMenu(): Promise<void> {
  // Show a simple prompt
  let segmentPrompt = new Prompt();
  segmentPrompt.title = "Segmented Task Management";
  segmentPrompt.message = "Choose a specialized flow to manage tasks:";
  segmentPrompt.addButton("Manage Overdue (Segmented)");
  segmentPrompt.addButton("Manage Today No-Time (Segmented)");
  segmentPrompt.addButton("Cancel");

  if (!segmentPrompt.show() || segmentPrompt.buttonPressed === "Cancel") {
    log("User canceled segmented tasks menu.");
    script.complete();
    return;
  }

  switch (segmentPrompt.buttonPressed) {
    case "Manage Overdue (Segmented)":
      await manageOverdueTasksSegmented();
      break;
    case "Manage Today No-Time (Segmented)":
      await manageTodayNoTimeTasksSegmented();
      break;
    default:
      log("Unexpected button pressed in segmented tasks menu.");
      break;
  }
