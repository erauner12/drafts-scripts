...
var require_lodash = __commonJS((exports, module) => {
  (function() {
    ...
    var runInContext = function runInContext(context2) {
      ...
      var { ceil: nativeCeil, floor: nativeFloor } = Math2, nativeGetSymbols = Object2.getOwnPropertySymbols, nativeIsBuffer = Buffer ? Buffer.isBuffer : undefined2, nativeIsFinite = context2.isFinite, nativeJoin = arrayProto.join, nativeKeys = overArg(Object2.keys, Object2), nativeMax = Math2.max, nativeMin = Math2.min, nativeNow = Date2.now, nativeParseInt = context2.parseInt, nativeRandom = Math2.random, nativeReverse = arrayProto.reverse;
      var DataView = getNative(context2, "DataView"), Map = getNative(context2, "Map"), Promise2 = getNative(context2, "Promise"), Set = getNative(context2, "Set"), WeakMap2 = getNative(context2, "WeakMap"), nativeCreate = getNative(Object2, "create");
...
`;
  ...
  async performAction() {
    try {
      ...
      const p2 = new Prompt;
      p2.title = "GitHub Item Actions";
      p2.message = "What would you like to do with this GitHub item?";
      p2.addButton("Open in Browser");
      p2.addButton("Export All Information");
      p2.addButton("AI Actions");
      p2.addButton("Cancel");
      const result2 = p2.show();
      if (!result2 || p2.buttonPressed === "Cancel") {
...
Draft: ${draftLink}`;
  ...
  async performAction() {
    try {
      ...
      const p2 = new Prompt;
      p2.title = "Jira Issue Actions";
      p2.message = "What would you like to do with Jira issue " + this.issueKey + "?";
      p2.addButton("Open in Browser");
      p2.addButton("Export All Information");
      p2.addButton("AI Actions");
      p2.addButton("Create Todoist Task");
      p2.addButton("Add Comment");
      p2.addButton("Cancel");
      const result2 = p2.show();
      if (!result2 || p2.buttonPressed === "Cancel") {
        console.log("User cancelled the action.");
      ...
      console.log("User selected action:", p2.buttonPressed);
      switch (p2.buttonPressed) {
        case "Open in Browser":
          this.openInBrowser();
          break;
...
class TodoistTask extends SourceItem {
  ...
  formatCommentsForExport(comments, includeTimestamps) {
...
`;
  ...
  async addComment() {
    try {
      ...
      if (commentId) {
        const result2 = this.todoist.updateComment(commentId, {
          content: commentText
        });
        if (result2 && result2.id) {
          ...
          console.error("Failed to update comment on Todoist task:", result2, this.todoist.lastError);
          app.displayErrorMessage("Failed to update comment on Todoist task.");
          failAction("An unexpected error occurred during execution");
        }
      } else {
        const result2 = this.todoist.createComment({
          task_id: this.taskId,
          content: commentText
        });
        if (result2 && result2.id) {
          ...
          console.error("Failed to add comment to Todoist task:", result2, this.todoist.lastError);
  ...
  openChatGPTWithClipboard(refinedPrompt) {
    try {
      ...
      prompt.message = `You have a refined prompt and possibly existing clipboard text.
...
${allComments}`;
  ...
  async performAction() {
    try {
      ...
      const p2 = new Prompt;
      p2.title = "Todoist Task Actions";
      p2.message = "What would you like to do?";
      p2.addButton("Open in Browser");
      p2.addButton("Export All Information");
      p2.addButton("AI Actions");
      p2.addButton("Add or Edit Comment");
      p2.addButton("Export to New Draft");
      p2.addButton("Export and Delete");
      p2.addButton("Open ChatGPT (Clipboard)");
      p2.addButton("Compose ChatGPT Prompt");
      p2.addButton("Start Session for Evan");
      p2.addButton("Delete Task");
      p2.addButton("Cancel");
      const result2 = p2.show();
      if (!result2 || p2.buttonPressed === "Cancel") {
        console.log("User cancelled the action.");
      ...
      console.log("User selected action:", p2.buttonPressed, "on TodoistTask performAction.");
      switch (p2.buttonPressed) {
        case "Open in Browser":
          this.openInBrowser();
          break;
...
async function runSourceIntegration() {
  try {
